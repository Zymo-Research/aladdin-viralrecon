...
include { KRAKEN2_UNCLASSIFIED_REMAP as KRAKEN2_REMAP } from '../modules/local/kraken2_viral_remap'
...
include { FASTQ_ALIGN_BWAMEM2     } from '../subworkflows/local/align_bwa'
...
workflow ILLUMINA {
    ...
    //
    // MODULE: Run Kraken2 for the taxonomy classification of de-host sequence/aligned sequence (RNAseq)
    //
    ...
    //
    // SUBWORKFLOW: Alignment with Bowtie2/Bwamem2
    //
    ch_bam                      = Channel.empty()
    ch_bai                      = Channel.empty()
    ch_bowtie2_multiqc          = Channel.empty()
    ch_bwamem2_multiqc          = Channel.empty()
    ch_bowtie2_flagstat_multiqc = Channel.empty()
    ch_bwamem2_flagstat_multiqc = Channel.empty()
    if (!params.skip_variants && params.aligner == 'bowtie2') {
        ...
        FASTQ_ALIGN_BWAMEM2 (
            ch_variants_fastq,
            PREPARE_GENOME.out.bwamem_index,
            params.save_unaligned,
            false,
            PREPARE_GENOME.out.fasta.map { [ [:], it ] }
        )
        ch_bam                      = FASTQ_ALIGN_BWAMEM2.out.bam
        ch_bai                      = FASTQ_ALIGN_BWAMEM2.out.bai
        ch_bwamem2_multiqc          = FASTQ_ALIGN_BWAMEM2.out.log_out
        ch_bwamem2_flagstat_multiqc = FASTQ_ALIGN_BWAMEM2.out.flagstat
    ...
    if (!params.skip_multiqc) {
        ...
        MULTIQC (
            ...
            ch_bwamem2_multiqc.collect{it[1]}.ifEmpty([]),
            ch_bwamem2_flagstat_multiqc.collect{it[1]}.ifEmpty([]),
