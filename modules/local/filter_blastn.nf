process FILTER_BLASTN {
    ...
    def min_pident = task.ext.min_pident ?: 98

    """
    cat $header $hits > ${prefix}.results.blastn.txt
    awk 'BEGIN{OFS=\"\\t\";FS=\"\\t\"}{print \$0,\$6/\$16,\$6/\$15}' $hits | awk 'BEGIN{OFS=\"\\t\";FS=\"\\t\"} \$16 > ${min_contig_length} && \$18 > ${min_perc_contig_aligned} && \$1 !~ /phage/ && \$5 > ${min_pident} {print \$0}' > tmp.out
